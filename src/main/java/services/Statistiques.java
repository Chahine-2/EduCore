...
public class Statistiques {
    ...
    public Map<String, Integer> getReservationsParMateriel() {
        ...
        String req = "SELECT m.nom, COUNT(r.id) as total " +
                "FROM materiel m LEFT JOIN reservationmateriel r ON m.id = r.materiel_id " +
    ...
    public Map<String, Integer> getReservationsParStatut() {
        Map<String, Integer> map = new LinkedHashMap<>();
        String req = "SELECT statut, COUNT(*) as total FROM reservationmateriel GROUP BY statut";
        try {
            Statement stm = MyDataBase.getInstance().getCnx().createStatement();
            ResultSet rs = stm.executeQuery(req);
            while (rs.next()) {
    ...
    public void materielPlusDemande() {
        String req = "SELECT m.nom, COUNT(r.id) as total " +
                "FROM materiel m JOIN reservationmateriel r ON m.id = r.materiel_id " +
    ...
    public void reservationsMoisCourant() {
        String req = "SELECT COUNT(*) as total FROM reservationmateriel " +
